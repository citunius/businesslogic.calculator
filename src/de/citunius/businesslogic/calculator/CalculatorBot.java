...
public class CalculatorBot {
    ...
    public CalculatorBot(String tenantId, String accountId, HashMap<String, String> pluginMap) {
        logger.info("called");
	...
	public String handleIncomingMessage(String tenantId, String accountId, String jsonMessage, boolean anonymousUserAccountExists, String jsonAnonymousUserAccount, boolean mobileUserAccountExists, String jsonMobileUserAccount) {		
    	...
    	if (message.getText() != null && message.getText().length() != 0) {
    		...
    		if (cs.isMathTask(message.getText())) {
    			logger.debug("User message contains math"); 
    			double result;
				try {
					result = cs.calculate(message.getText());
					replyMessage = ls.getString("Calculator.Result", defaultLanguage)+": "+result;
				} catch (Exception e) {
					logger.error(e);
				}    			
    		} else {
    			logger.info("User message does not contains math.");
    			replyMessage = ls.getString("Calculator.NoMath", defaultLanguage);
    		}
    	} else {
    		logger.info("User message does not contains math.");
    		replyMessage = ls.getString("Calculator.NoMath", defaultLanguage);
